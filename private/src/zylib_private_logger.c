...
ZYLIB_NONNULL
static inline uint64_t zylib_private_logger_send_message(const zylib_private_logger_t *logger,
                                                         zylib_logger_severity_t severity, const char *file_name,
                                                         uint64_t line_number, const char *function_name,
                                     ...
                                     FILE *file, zylib_logger_format_t format, zylib_logger_filter_t filter)
{
    ...
    r = zylib_private_allocator_malloc(alloc, sizeof(zylib_private_logger_t), (void **)logger);
    if (r)
    {
                                    ...
                                    const char *format, ...)
{
    if (logger->filter(severity))
    {
        ...
        if (!zylib_private_allocator_malloc(logger->allocator, ZYLIB_LOGGER_MAX_MESSAGE_SIZE,
        ...
        r = zylib_private_logger_send_message(logger, severity, file_name, line_number, function_name, display_message);
    error:
        if (display_message != NULL)
        {
            zylib_private_allocator_free(logger->allocator, (void **)&display_message);
        }
        return r;
    }
    return 0;
